...
class LinkedListArray {
    ...
    public boolean remove(int pozicija) {
        ...
        if (isNull(iter) && iter.next != null) {
            if (iter == first) {
                first = iter.next;
                iter = first;
            } else {
                LinkedListElement prev = findPrev(iter);
                iter.setNext(prev, iter.next);
            }
        }
        ...
        LinkedListElement iter_next = iter.next;
        int i = st_elementov;
        int j = 0;
        if (st_elementov < N / 2 && !isNull(iter_next)) {
            ...
            if (isNull(iter_next))
                iter.setNext(iter, iter_next.next);
        }
    ...
    public boolean insert(int vrednost, int pozicija) {
        ...
        if (hasNull(iter)) {
            ...
            if (index1 / N != ix_clen) {
                ...
                add_node(N, iter, iter.next);
                int st_elementov = N / 2;
                int a = st_elementov;
                if (st_elementov % 2 == 1)
                    a++;

                LinkedListElement iter_new = iter.next;
